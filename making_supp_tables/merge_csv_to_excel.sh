...
for csv_file in glob.glob("*.csv"):
    # Extract the tab name from the filename by joining the first two segments before underscores
    tab_name = "_".join(csv_file.split("_")[:2])
    df = pd.read_csv(csv_file)
    worksheet = workbook.add_worksheet(tab_name[:31])  # Excel tab names limit to 31 chars
